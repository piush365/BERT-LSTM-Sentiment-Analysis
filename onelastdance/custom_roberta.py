...
import sys
sys.path.append("D:/onelastdance")  # Adjust to your project path
...
class RobertaLayer(nn.Module):
    def __init__(self, config):
        ...
        if self.add_cross_attention:
            if not self.is_decoder:
                raise ValueError("Cross-attention layers should only be used in decoder mode.")
            self.crossattention = RobertaAttention(config, position_embedding_type="absolute")
        
        # Adding LSTM layer after self-attention
    ...
    def forward(
        ...
        # Passing self-attention output through LSTM
        lstm_output, _ = self.lstm(attention_output)
        
        # Merging bidirectional outputs
        lstm_output = lstm_output[:, :, :lstm_output.shape[2] // 2] + lstm_output[:, :, lstm_output.shape[2] // 2:]
        ...
        if self.is_decoder and encoder_hidden_states is not None:
            ...
            lstm_output = cross_attention_outputs[0]
        
        # Feed-forward network
        layer_output = self.output(self.intermediate(lstm_output), lstm_output)
...
class RobertaWithLSTM(RobertaModel):
    def __init__(self, config):
        super().__init__(config)

        for i, layer in enumerate(self.encoder.layer):
            layer.lstm = nn.LSTM(config.hidden_size, config.hidden_size // 2,
        ...
        # Initialize transformer weights from scratch
    ...
    def _init_weights(self, module):
        if isinstance(module, nn.Linear):
            ...
            init.normal_(module.weight, mean=0, std=self.config.hidden_size ** -0.5)
        elif isinstance(module, nn.LayerNorm):
